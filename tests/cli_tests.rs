...
fn test_list_command() {
    ...
    assert!(cmd.status.success(), "Command failed with stderr: {}", stderr);
    
    // Check that output contains expected elements (adjust as needed)
    assert!(stdout.contains("src"), "Output should contain 'src' directory");
...
fn test_list_with_path() {
    ...
    // Check that output contains expected elements (adjust as needed)
    assert!(stdout.contains("main.rs"), "Output should contain 'main.rs' file");
}
...
fn test_list_nonexistent_path() {
    ...
    assert!(stderr.contains("does not exist"), "Error message should mention that path does not exist");
}
...
fn test_color_output() {
    ...
    assert!(stdout.contains("src") || stdout.contains("Cargo.toml"), 
...
fn test_list_tree() {
    ...
    assert!(stdout.contains("├──") || stdout.contains("└──"), "Output should contain tree structure characters");
...
fn test_list_tree_depth_1() {
    ...
    if stdout.contains("src") {
        ...
        let next_line = stdout.lines().skip_while(|line| !line.contains("src")).nth(1).unwrap_or("");
        assert!(!next_line.starts_with("    ") || !next_line.contains("├──") && !next_line.contains("└──"), 
